...
### App Extension 디버깅 방법

푸시 노티를 인터셉트해 별도의 처리를 하거나, 위젯이 정상적으로 처리되고 있는지 디버깅 해보고 싶을 경우가 있다.

이런 App Extension 에 디버거를 붙이는 방법은 두 가지가 있다.
...
#### App Extension `scheme` 으로 실행

xcode 상단 중앙에 있는 `scheme` 중 App Extension 에 해당하는 스킴을 선택해 실행한다.

만약 App Extension 스킴이 없다면 `New Scheme` 으로 생성한 다음,  
Run 섹션의 Executable 만 `Paranet App` 으로 설정하면된다  
(설정해두지않으면 매 빌드마다 선택해야함)

</br>

#### Attach to Process

기존 앱 스킴을 실행하고 있는 상태에서, Xcode > Debug > Attach to Process > App Extension 에 해당하는 타겟 선택
...
Xcode 15 부터 지원하는 Preview 기능은 최소 타겟 iOS 17 이상을 지원하는 앱에서 동작한다

만약 기존의 플랫폼을 지원해야하는 레거시 프로젝트에서는 `@available` 어노테이션을 사용하면 미리보기가 정상적으로 동작한다.
